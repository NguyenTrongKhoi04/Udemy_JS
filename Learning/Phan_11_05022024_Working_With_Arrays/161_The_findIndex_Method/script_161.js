 ...
 * TODO 1) Use "inlcudes()" tìm trong arr "movements" có giá trị "-130" k? Nếu có thì in ra true và ngược lại
 * TODO 2) TÌm xem có giá trị nào trong arr "movements" > 1500 k? nếu có thì in "true" và ngc lại
 * TODO 3) use "some()" để làm phần 1)
 * TODO 4) Viết func vay nợ - "loan trong Bankist App".
 *          ! Số tiền vay > 0
 *          ! 10% Số tiền vay phải <= số tiền lớn nhất của giao dịch
 *          ! Sau khi vay sẽ update luôn bên list danh sách chuyển khoản
 * TODO 5) Kiểm tra xem tất cả phần tử của "account4.movements" > 0 k? Nếu có thì in ra true và ngược lại
 *          ! sử dụng Separate callbak 
 */
...
// TODO 2)
console.log(movements.some(value => value>1500));
...
btnLoan.addEventListener('click', function(e){
    ...
    if(amount > 0 && currentA4ccount.movements.some(value => value >= amount*0.1)){
      currentAccount.movements.push(amount);
...
// TODO 5)
    /**
     * * Nếu som() chỉ cần 1 phần tử thỏa mãn điều kiện thì "every()" cần all phần tử thỏa mãn
     * ? Separate callback là tách riêng hàm callback ra
     */
const funcCallBack = mov => mov>0;
console.log(account4.movements.every(funcCallBack));